...
| `autoresearch/distilled/coaching_adapter_r4_s2/` | R4 S2 adapter（已刪除，R4 DPO 為最終版） |
| `autoresearch/distilled/coaching_adapter_r4_dpo/` | **R4 DPO adapter（88.3 ± 1.4, 6 runs 驗證）** |
...
| `autoresearch/distilled/coaching_adapter_v3_dpo_v5/` | V5 DPO adapter（87.9 avg，已刪除） |
...
| `autoresearch/distilled/coaching_adapter_r4_dpo/` | **當前最佳 adapter**（88.3 ± 1.4, 6 runs, Eval V2） |
...
## Part 8：2026-03-16 資源盤點與恢復

### 8.1 Adapter 流失事件回顧

原始最佳 adapter `coaching_adapter_v3_dpo`（88.5 ± 0.6）在 R5 蒸餾實驗中被覆蓋。
Lottery training 嘗試恢復，最高只達 80.2（transformers 5.0.0, 2-3 輪）。

**轉機**：發現 R4 DPO adapter 仍然存在，經 6 次穩定性驗證確認分數。

### 8.2 R4 DPO Adapter 穩定性驗證（6 runs）

**推論參數**：`temp=0.01, rep_penalty=1.0, top_p=0.9`
**環境**：transformers 5.2.0, trl 0.29.0, peft 0.18.1, torch 2.10.0
**Checksum**：`a1fb43fafb856fe57f14bfa64e9adcce` (adapter_model.safetensors)

| Run | Composite | coach_ratio | reflection | technique | os_inquiry |
|:---:|:---------:|:-----------:|:----------:|:---------:|:----------:|
| 1 | 89.3 | 40.3% | 90.5% | 3.8 | 1.7 |
| 2 | 88.0 | 38.6% | 88.3% | 3.8 | 1.5 |
| 3 | 87.6 | 39.8% | 88.3% | 3.7 | 1.5 |
| 4 | 86.4 | 38.8% | 88.7% | 3.5 | 1.3 |
| 5 | 88.0 | 39.3% | 88.3% | 3.8 | 1.5 |
| 6 | 90.7 | 40.3% | 91.1% | 4.0 | 1.8 |
| **avg** | **88.3 ± 1.4** | 39.5% | 89.2% | 3.8 | 1.6 |

**與原始 v3_dpo 對比**：

| | 原始 v3_dpo (已失) | R4 DPO (現存) |
|---|:---:|:---:|
| avg | 88.5 ± 0.6 | **88.3 ± 1.4** |
| range | 87.9 — 89.1 | 86.4 — 90.7 |
| reflection | 85% | **89.2%** (+4%) |
| os_inquiry | 2.0 | 1.6 (-0.4) |

**結論**：R4 DPO 確認為正式基線，平均分差距僅 0.2，在統計誤差內。

### 8.3 推論參數修正

**Bug 發現**：`eval_coaching.py` 預設推論參數錯誤導致分數嚴重低估。

| 設定 | avg（10 runs） | 問題 |
|------|:-----------:|------|
| temp=0.7, rep=1.1 | 76.7 ± 5.0 | eval 預設值錯誤 |
| **temp=0.01, rep=1.0** | **88.0 ± 3.1** | **正確推論參數** |

**修正內容**：
- `eval_coaching.py`：`TEMPERATURE` 0.7→0.01, `REP_PENALTY` 1.1→1.0, `DEFAULT_ADAPTER` v3_dpo→r4_dpo
- Checksum 修正後：`eval_coaching.py` 原 `b46f8c5b...`

**正確推論參數（永久記錄）**：
```
temperature = 0.01     # 極低溫度確保一致性
repetition_penalty = 1.0  # 無懲罰 — 反映技巧需要重複客戶用詞
top_p = 0.9            # 影響極小
```
...
| 類別 | 數量 | 大小 | 理由 |
|------|:----:|:----:|------|
| v3_dpo 失敗變體 | 23 | 35G | 全部低於 R4 DPO |
| v3_s2 失敗變體 | 11 | 14G | 中間產物，s2_best 已備份 |
| exp_a-d adapter | 8 | 9.6G | 四方向實驗全敗 (64.9-82.0) |
| 7B adapter | 3 | 2.2G | 7B 已放棄 (63.7-74.9) |
| R4 S2 / 冗餘備份 / v1 / v3 | 4 | 3.9G | 過時或已有備份 |

**保留的 adapter（4 個）**：

| Adapter | 分數 | 用途 |
|---------|:----:|------|
| `coaching_adapter_r4_dpo` | 88.3 ± 1.4 | **正式基線** |
| `coaching_adapter_v3_dpo` (= `_best`) | 80.2 | lottery 備用 |
| `coaching_adapter_v3_s2_best` | — | 可重跑 DPO |

**備份位置**：`backups/20260316/`（4.5GB，含 CHECKSUMS.md5）
...
GPU 配置：
  GPU 0: RTX 3080 Ti 12GB
  GPU 1: RTX 3080 10GB
  GPU 2: RTX 3080 10GB（eval 用）
  GPU 3: RTX 3080 12GB（eval 用）
```

---

## Phase 5 路線圖（2026-03-16 起）

**當前狀態**：coaching_adapter_r4_dpo = **88.3 ± 1.4**（Eval V2, 6 runs），Orchestration Haiku 87% ± 7.5%

> 注意：eval_coaching.py 自 2026-03-15 起統一使用 V2 權重。所有分數均為 V2。
> 原始 v3_dpo 在 V2 下為 91.7（單次），R4 DPO 為 88.3 ± 1.4（6 runs），差距 3.4。

### Step 1：Eval 驗證 R4 adapter ✅ 完成
- R4 DPO 驗證：88.3 ± 1.4（6 runs, 另一 session）/ 88.1 ± 0.9（5 runs, 本 session）
- 推論參數 bug 已修正：eval 預設 temp 0.7→0.01, rep_penalty 1.1→1.0
- 資源清理完成：70G→7G
...
**測試 Session 3（修正後第二輪）— 模板卡死**：

部署配置同 Session 2（commit 40a096a：反複述禁令 + temp=0.1 + STT null bytes 清理）。

| Turn | User | Coach | 問題 |
|------|------|-------|------|
| 1 | hi | hi. | — |
| 2 | 你好！教練 | 你現在是什麼狀態？ | ✅ 正常 |
| 3 | 對工作有疑問 | 你提到「對工作有疑問」。你今天想要探索什麼？ | ✅ 可接受 |
| 4 | 關於新的職涯選擇 | 「關於新的職涯選擇」。你最想成為什麼樣的人？ | 仍複述開頭 |
| 5 | 可以擁有很多技能的人 | 「可以擁有很多技能的人」。你提到「擁有很多技能」——這背後是什麼？ | 複述開頭 |
| 6 | 很有能力吧 | 很有能力。這份「很有能力」，是對自己的什麼感受？ | 複述開頭 |
| 7 | 目前沒什麼能力啊！ | 你說「目前沒什麼能力」。這份「沒什麼能力」，在保護你什麼？ | 複述+**模板 A** |
| 8 | 懶得學習吧！！ | 你說「懶得學習」。這份「懶得學習」，在保護你什麼？ | 複述+**模板 A 重複** |
| 9 | 不知道 | 你說不知道。這份不知道，背後是什麼？ | **模板 B** |
| 10 | 就不知道啊 | 你說「不知道」。這份不知道，背後是什麼？ | **模板 B 重複** |

**進步**（vs Session 1 & 2）：
- ✅ 幻覺消失（不再把「你好」聽成「好累」）
- ✅ 對話能推進到 10 turns（Session 1 只有 8 turns 且全卡死在「你最想帶走什麼？」）
- ✅ STT null bytes 已清理
- ✅ Haiku 推理品質高：偵測到「我沒能力 vs 我懶得學習」的身份/行為矛盾

**持續問題**：
1. **複述開頭**（7/10 回應）：反複述禁令降低了逐字複述的程度（從 100%→70%），但模型仍偏好以客戶原話開頭
2. **模板卡死**：「這份 X，在保護你什麼？」「這份 X，背後是什麼？」各連續出現 2 次
3. **技巧單一**：全程只有「反映+提問」，零挑戰、零重構、零留白、零底線功
4. **hint 遵循能力不足**：Haiku 偵測到 beliefs 矛盾但 3B 未執行 brain hacking

**三次測試趨勢**：

| | Session 1 (pre-fix) | Session 2 (post-fix) | Session 3 (post-fix) |
|--|:---:|:---:|:---:|
| 逐字複述 | 100% | 0%（改為幻覺） | 70%（降低但未消除） |
| 幻覺 | 無 | 嚴重（「你好」→「好累」） | 無 |
| 問題卡死 | 3 次同問題 | 3 次同問題 | 2 次同問題×2 模板 |
| 技巧多樣性 | 零 | 零 | 零 |
| 對話推進 | 無（8 turns 原地打轉） | 無（4 turns 即崩潰） | 有限（10 turns，有深化方向） |
| 可用性 | ❌ 不可用 | ❌ 不可用 | ⚠️ 勉強可讀但不可用 |

**結論更新**：

> 參數調整（反複述禁令 + temp 0.01→0.1）帶來了增量改善：消除幻覺、降低逐字複述、
> 延長對話長度。但 3B 模型的核心限制未變：
>
> 1. **Generation diversity 不足**：模型學到了 2-3 個「安全模板」並反覆使用
> 2. **Hint 遵循能力弱**：coach_hint 的自然語言指令被忽略（3B 從未在此格式下訓練）
> 3. **短輸入 OOD**：eval 場景用 30-80 字 user turns，真實語音多為 1-10 字
>
> **參數調整已觸頂，進一步改善需要模型層或架構層介入。**

**決策：暫停 local 引擎上線**（commit bb14f2d）。引擎選擇 UI 已隱藏，所有用戶自動使用 Cloud (Opus) 引擎。待以下任一條件達成後重新啟用：
- S2 在 hybrid prompt 格式 + 短輸入場景下重訓
- 或改用更大模型（7B+）嘗試 hint 遵循
- 或改為 Haiku 全程生成（放棄本地推理）
