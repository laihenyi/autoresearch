...
- **結果（9 runs, transformers 5.2.0）**：**77.2 ± 5.0**（range 70.0-85.3）
- Eval V2 方差極大（±5），單次跑不可靠，需 ≥5 次取平均
- 另一 session 報的 88.3 ± 0.9（3 runs）為高方差下的幸運結果
- **注意**：transformers 5.0.0 → 5.2.0 推論結果不同（5.0.0 更低）
- **結論**：R4 adapter 真實水準 ~77，距離 V2 = 91.7 仍有差距
