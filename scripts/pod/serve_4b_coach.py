...
def _tokenize_for_generation(messages: list[dict]) -> tuple:
    ...
    if args.structured:
        # In structured mode, model outputs: response + [INTERNAL]...[/INTERNAL]
        text += "<|im_start|>assistant\n"
    else:
        # Add assistant turn without <think> to skip reasoning
        text += "<|im_start|>assistant\n"
...
def _sync_response(
    ...
    # Generate — coach responses are 15-30 chars (80 tokens), but [INTERNAL]
    # blocks need ~300-500 tokens
    effective_max_tokens = 512 if args.structured else 80
    with torch.no_grad():
        out = model.generate(
            **inputs,
    ...
    # Two-pass generation: if structured mode and no [INTERNAL] block,
    # append the marker and continue generating
    if args.structured and "[INTERNAL]" not in raw_text:
        continuation = "\n\n[INTERNAL]\n"
        cont_input = tokenizer(
            tokenizer.decode(out[0], skip_special_tokens=False).rstrip("<|im_end|>") + continuation,
            return_tensors="pt"
        ).to(model.device)
        cont_len = cont_input["input_ids"].shape[1]
        with torch.no_grad():
            cont_kwargs = dict(
                **cont_input,
                max_new_tokens=300,
            ...
            out2 = model.generate(**cont_kwargs)
        cont_ids = out2[0][cont_len:]
        cont_text = tokenizer.decode(cont_ids, skip_special_tokens=True)
        raw_text = raw_text + continuation + cont_text

    # Apply meta-commentary filter (skip in structured mode)
    if args.structured:
