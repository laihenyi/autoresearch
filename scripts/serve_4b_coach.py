...
# H4: Multi-question check — 2+ real questions (exclude confirmation tags like 是這樣嗎？)
_CONFIRM_TAGS = re.compile(r"是這樣嗎|對嗎|是嗎|對不對|好嗎")
# H4: Meta-explanation check — coach explaining why they asked
_META_EXPLAIN = re.compile(
    r"我[之所以這].*?[問是因]|之所以|我問這個是|"
    r"我想了解.*?所以|我注意到.*?所以我"
)
...
class CriticLoop:
    ...
    CORRECTIVE_HINTS = {
        ...
        "multi_question": "（系統提示：你一次問了太多問題。請只問一個問題，或只做反映。）",
        "meta_explanation": "（系統提示：不要解釋你為什麼這樣問。直接反映或提問，不加後設說明。）",
    }
    ...
    def check(response: str, turn_idx: int) -> str | None:
        ...
        if _CRITIC_EVAL.search(unquoted):
            return "evaluation"
        # H4: Multi-question — count real questions (exclude confirmation tags)
        questions = [q for q in unquoted.split("？") if q.strip() and not _CONFIRM_TAGS.search(q)]
        if len(questions) >= 2:
            return "multi_question"
        # H4: Meta-explanation — coach explaining why they asked
        if _META_EXPLAIN.search(unquoted):
...
def _sync_response(
    ...
    if args.structured:
        ...
        # H6: Phase-aware temperature scheduling
        phase = _estimate_phase(messages)
        _PHASE_TEMPS = {
            "opening": 0.7,
            "exploring": 0.6,
        ...
        gen_temp = _PHASE_TEMPS.get(phase, req.temperature)

        # Generate + critic check (max 2 attempts)
        _, coach_response = _generate_once(gen_messages, 80, gen_temp, req.top_p)
        ...
        if violation:
            ...
            _, retry_response = _generate_once(retry_messages, 80, gen_temp, req.top_p)
